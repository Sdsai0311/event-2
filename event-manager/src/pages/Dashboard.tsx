...
import { Plus, Search, Calendar as CalendarIcon, Users, Wallet, TrendingUp, ArrowRight, Star, QrCode } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
    ...
    const filteredEvents = events.filter(event => {
        ...
        if (user?.role === 'student') {
            // Students see only approved events that are current or upcoming
            const isApproved = event.status === 'confirmed' || event.isApproved;
            const isFutureOrToday = event.date >= today;
            return matchesSearch && matchesType && isApproved && isFutureOrToday;
        }
    ...
    if (isLoading && events.length === 0) {
        return (
            <div className="space-y-8 animate-in text-slate-900">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
                        {collegeName} <span className="text-indigo-600">{isAdmin ? 'Admin Portal' : 'Student Portal'}</span>
                    </h1>
                    <p className="text-slate-500 mt-1 font-medium">
                        {isAdmin ? 'Manage college events and approvals with ease.' : 'Explore upcoming events and check your attendance.'}
                    </p>
                </div>
                {isAdmin && (
                    <Button
                    ...
                    <Card className="p-6 relative overflow-hidden group hover:ring-2 hover:ring-indigo-500/20 transition-all">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <h3 className="text-3xl font-black text-slate-900 mt-1">{upcomingEventsCount}</h3>
                            </div>
            ...
            {filteredEvents.length === 0 ? (
                <Card className="py-24 text-center glass-card border-none">
                    ...
                    <p className="text-slate-500 max-w-sm mx-auto mt-2 font-medium">
                        {isAdmin ? 'Try adjusting your filters or create something spectacular from scratch.' : 'Check back later for exciting college events!'}
                    </p>
                </Card>
            ) : (
                            ...
                            <div key={event.id}>
                                <Card className="hover:shadow-2xl hover:shadow-indigo-100 hover:-translate-y-1 transition-all group overflow-hidden border-0 ring-1 ring-slate-200/60 bg-white p-0 h-full flex flex-col">
                                    <div className="h-48 bg-slate-100 relative overflow-hidden">
                                        ...
                                        {isToday && (
                                            <div className="absolute top-4 left-4 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md flex items-center animate-pulse">
                                    ...
                                    <div className="p-6 flex-1 flex flex-col space-y-6">
                                        <div className="grid grid-cols-2 gap-6 pt-2">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <div className="flex items-center text-sm font-bold text-slate-700">
                                                    <CalendarIcon className="h-4 w-4 mr-2 text-emerald-500 shrink-0" />
                                        ...
                                        <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                                            ...
                                            {isAdmin ? (
                                                <Link to={`/events/${event.id}`} className="flex items-center text-indigo-600 font-black text-xs group-hover:translate-x-1 transition-transform">
                                                    Manage <ArrowRight size={14} className="ml-1" />
                                                </Link>
                                            ) : (
                                                <div className="flex gap-2">
                                                    {isToday ? (
                                                        <Button size="sm" className="bg-emerald-600 text-white text-[10px] py-1 px-3">
                                                            <QrCode size={12} className="mr-1" /> Check-in
                                                        </Button>
                                                    ) : (
                                                        <Button
                                                            size="sm"
                                                            className="premium-gradient text-white text-[10px] py-1 px-3"
